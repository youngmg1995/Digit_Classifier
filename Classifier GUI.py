# -*- coding: utf-8 -*-
"""
Created on Mon Mar 23 14:18:41 2020

@author: Mitchell

Classifier GUI.py
~~~~~~~~~~~~~~~~~
An interactive GUI for classifying user inputted handwritten digits using simple
neural networks constructed using numpy. The GUI allows the user to draw an
image on the canvas and then select one of two simple ANN models to classify 
the image as a digit in 0-9. The user has the option of choosing between two
...
Note that this file does not require any advanced machine learining modules. It
only requires numpy and some additional libraries, such as tkinter, PIL, etc.,
for building the GUI and manipulating the images.

The script used to construct and save the below networks is included in the
project under the file name "Network_Builder.py".
...
import Network_Builder.network
...
filename_1 = 'network1.json'
net_1 = Network_Builder.network.load(folder + filename_1)
print('Naive NN loaded')

filename_2 = 'network2.json'
net_2 = Network_Builder.network.load(folder + filename_2)
...
def image_resize(img):
    ...
    img2 = np.array(img2).reshape((28**2,1))
    return img2
...
def classify():
    ...
    if selected_classifier == 1:
        result = net_1.feedforward(image2)
    elif selected_classifier == 2:
        result = net_2.feedforward(image2)
...
radio1 = tkinter.Radiobutton(right_frame, text='Naive NN', font=('TkDefaultFont', 14), value = 1, variable = selected, pady = 10)
radio2 = tkinter.Radiobutton(right_frame, text='Optimized NN', font=('TkDefaultFont', 14), value = 2, variable = selected, pady = 10)

radio1.grid(column = 0, row = 1, columnspan = 5)
radio2.grid(column = 5, row = 1, columnspan = 5)
